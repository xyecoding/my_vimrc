...
function! s:viewer._start(file) dict abort " {{{1
  ...
  let tex =  vimtex#util#shellescape(expand('%:p'))
  "let tex =  'C:\Users\12197\AppData\Local\Packages\CanonicalGroupLimited.Ubuntu20.04onWindows_79rhkp1fndgsc\LocalState\rootfs' .. trim(tex, "'")
  "let tex =  '/C/Users/12197/AppData/Local/Packages/CanonicalGroupLimited.Ubuntu20.04onWindows_79rhkp1fndgsc/LocalState/rootfs' .. trim(tex, "'")
  let l:cmd = substitute(l:cmd, '@tex',
        \ tex , 'g')
  let l:cmd = substitute(l:cmd, '@pdf', vimtex#util#shellescape(l:file), 'g')
  "echo vimtex#util#shellescape(l:file)
  "echo vimtex#util#shellescape(expand('%:p'))
  "echo l:cmd
  "sleep 50
